...
消毒水行动是一个数据采集项目，旨在为网页应用提供淘宝商品数据源。当前已可批量爬取店铺商品，保存完整信息与图片，并在本地完成 OCR。
...
- Python 3.13+
- Chrome 浏览器
- ChromeDriver（自动管理，缓存到 `chrome_profile/driver_cache`）
...
3. 可选：安装 Tampermonkey 辅助脚本（用于图片防盗链绕过）
- 运行 `python open_debug_chrome.py` 打开带调试端口的 Chrome（使用项目的 `chrome_profile`）
- 在弹出的浏览器中安装 Tampermonkey，并导入脚本文件：[`src/Tampermonkey/getimage.user.script`](src/Tampermonkey/getimage.user.script)
- 安装完成后关闭浏览器，爬虫会复用同一 profile 直接使用脚本的 API 下载图片
...
- `images/` - 主图和图文详情图片（命名防重名，附下载 manifest）
- OCR：完成图片下载后自动对主图/图文详情跑 OCR，结果写入 `product_data.json`
...
- `products/` - 每个商品的详细文件夹
- 每个商品目录内包含 `images/`（主图、图文详情已下载）及 OCR 结果
...
```json
{
  ...
  "styles": [
    {
      "style_name": "颜色/款式名称",
      "image_url": "款式图片本地路径（原始URL保存在 image_url_original）",
      "available": true,
      "sizes": [
        {
          "name": "尺码名称",
  ...
  "product_details": {
    ...
    "image_details": ["图文详情图片本地路径（原始 URL 保存在 image_details_original）"],
    "image_details_ocr": [
      {
        "image_url": "本地图片路径",
        "full_text": "OCR 结果",
    ...
    "main_images_ocr_text": "所有主图 OCR 文本汇总",
    "detail_images_ocr_text": "所有图文详情 OCR 文本汇总"
  }
}
```
